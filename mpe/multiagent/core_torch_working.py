...
class World(object):
    ...
    def step(self):
        ...
        p_force = torch.zeros(self.batch_dim, len(self.entities), 2)
    ...
    def integrate_state(self, p_force):
        for i, entity in enumerate(self.entities):
            ...
            if entity.max_speed is not None:
                speed = torch.sqrt(
                    entity.state.p_vel[0] ** 2 + entity.state.p_vel[1] ** 2
                )
    ...
    def get_collision_force(self, entity_a, entity_b):
        ...
        penetration = torch.logaddexp(torch.tensor(0.0), -(dist - dist_min) / k) * k
        force = (
            self.contact_force
            * delta_pos
            / dist.unsqueeze(-1)
