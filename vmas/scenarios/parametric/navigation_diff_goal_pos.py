...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.goal_positions = torch.nn.Parameter(
            torch.zeros(self.n_agents, world.dim_p, device=device, dtype=torch.float32)
        )
    ...
    def to_log(self) -> Dict:
        return {
            "distance_between_goals": torch.linalg.vector_norm(
                self.goal_positions_tanh, dim=-1
            ).mean(),
        }

    @property
    def goal_positions_tanh(self):
        return self.goal_positions.tanh()

    def reset_world_at(self, env_index: int = None):
        occupied_positions = self.goal_positions_tanh.unsqueeze(0).expand(
            self.world.batch_dim, self.n_agents, self.world.dim_p
        )
        ...
        for i, agent in enumerate(self.world.agents):
            ...
            agent.goal.set_pos(
                self.goal_positions_tanh[goal_index], batch_index=env_index
            )
    ...
    def reward(self, agent: Agent):
        ...
        if is_first:
            ...
            self.final_rew = torch.zeros_like(self.final_rew)
