...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.energy_reward_coeff = kwargs.get("energy_reward_coeff", 0.3)
    ...
    def process_action(self, agent: Agent):
        agent.consume_action = agent.action.u.clamp(min=0)
        ...
        if is_first:
            ...
            for a in self.world.agents:
                a.consumed_resources = torch.minimum(
                    a.consume_action.squeeze(-1), self.current_resources
                )
                ...
                a.energy_reward = (
                    -a.consume_action.squeeze(-1) * self.energy_reward_coeff
                )
