...
class Distances:
    def __init__(self, agents = None, left_boundaries = None, right_boundaries = None, boundaries = None, ref_paths = None, closest_point_on_ref_path = None, closest_point_on_left_b = None, closest_point_on_right_b = None, goal = None, obstacles = None):
...
def get_map_data(map_file_path, device=torch.device("cpu")):
...
def get_reference_paths(map_data):
    ...
    for ref_path_id in range(num_paths_all):
        reference_lanelets_index = get_reference_lanelet_index(ref_path_id + 1, path_to_loop) # Path ID starts from 1
    ...
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        if self.parameters.is_testing_mode:
            ...
            if self.parameters.training_strategy == "1":
                ...
                print("[INFO] Train on the entire map with prioritized experience replay")
            elif self.parameters.training_strategy == "3":
                print("[INFO] Train on the entire map with challenging initial state buffer")
