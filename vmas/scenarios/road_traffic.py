...
def calculate_reference_path(
    ...
    for lanelet in reference_lanelets_index:
        ...
        assert lanelets_share_same_boundaries is not None
...
render_title = "Multi-Agent Reinforcement Learning for Road Traffic (CPM Lab Scenario)"
...
# "1": Train on the entire map
# "2": Train on the entire map with prioritized replay buffer
# "3": Train on the entire map with challenging initial state buffer
# "4": Train on a specific scenario (e.g., intersection, merge-in, or merge-out)
...
class ScenarioRoadTraffic(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        if self.parameters.is_testing_mode:
            ...
            if self.parameters.training_strategy == "1":
                print("[INFO] Train on the entire map")
            elif self.parameters.training_strategy == "2":
                print(
                    "[INFO] Train on the entire map with prioritized experience replay"
                )
            elif self.parameters.training_strategy == "3":
                print(
                    "[INFO] Train on the entire map with challenging initial state buffer"
                )
            elif self.parameters.training_strategy == "4":
                print("[INFO] Train only on a partial map (intersection)")

        # Parameter adjustment to meet simulation requirements
        if self.parameters.training_strategy == "4":
            self.parameters.n_agents = 4
            print(f"[INFO] Changed the number of agents to {self.parameters.n_agents}")
        ...
        for i in range(self.n_agents):
            agent = Agent(
                ...
                color=tuple(torch.rand(3, device=device, dtype=torch.float32).tolist()),
                collide=False,
                render_action=False,
                u_range=[
                    max_speed,
        ...
        self.initial_state_buffer = (
            InitialStateBuffer(  # Used only when "training_strategy == '4'"
        ...
        for env_i in (
            ...
            if self.parameters.training_strategy in {"1", "2", "3"}:
                ...
                self.ref_paths_agent_related.scenario_id[
                    env_i, :
                ] = 0  # 0 for the whole map, 1 for intersection, 2 for merge-in, 3 for merge-out scenario
            else:
                if is_reset_single_agent:
            ...
            if (
                (self.parameters.training_strategy == "3")
                and (
                ...
                # print(f"[LOG] Reset with path ids: {initial_state[:, -2]}")
            else:
    ...
    def process_action(self, agent: Agent):
        """This function converts the actions generated by the policy to physical forces applied by VMAS, while adhering to the dynamics of the kinematic bicycle model."""
    ...
    def reward(self, agent: Agent):
        """
        Issue different rewards and penalties.
            Rewards:
                Moving forward
    ...
    def observation(self, agent: Agent):
        ...
        Returns:
            ...
            Specifically, the observation of this agent itself includes
        ...
        obs_others_list = [
            ...
            if is_observe_ref_path_other_agents
    ...
    def done(self):
        ...
        if (
            self.parameters.training_strategy == "3"
        ):  # Record into the initial state buffer
            if torch.rand(1) > (
                ...
                for env_collide in torch.where(is_collision_with_agents)[0]:
                    ...
                    # print(f"[LOG] Record states with path ids: {self.ref_paths_agent_related.path_id[env_collide]}")
