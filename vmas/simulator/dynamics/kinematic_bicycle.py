...
class KinematicBicycleDynamics:
    ...
    def process_force(self):
        ...
        velocity = self.agent.action.u[:, vmas.simulator.utils.X].clone()
        steering_angle = self.agent.action.u_rot.squeeze(-1).clone()
        # Ensure steering angle is within bounds
        steering_angle = torch.clamp(steering_angle, -self.max_steering_angle, self.max_steering_angle)
        ...
        if self.integration == "euler":
            new_state = self.euler(state, velocity, steering_angle)
        else:
